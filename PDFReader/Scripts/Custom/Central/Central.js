...
$(document).ready(function () {
    ...
    $(document).on('click', '#btnSearch', function () {
        ...
        if (projectList1.length > 0) {
            var xx1 = t(projectList1, "project_name");
            projectText = addColumn(xx1, "project_name");
        }
        ...
        if (companyList1.length > 0) {
            var xx1 = t(companyList1, "Company");
            companyText = addColumn(xx1, "Company");
        }
        ...
        //var finalText = `where (${projectText}) and (${companyText})`;

        debugger;
        //CentralTables.destroy();
        //CentralTables.init(finalText);
        $('#CentralTable').DataTable().ajax.reload();
    });
});
